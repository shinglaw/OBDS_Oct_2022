...
```{r}
dim(read10x_data) #columns are 5155 cells, rows are 33538 genes
```
...
```{r}
seurat_object[["orig.ident"]] #dataframe
```
...
```{r}
head(seurat_object$orig.ident) #vector
```
...
seurat_object[['percentage_mt']] <- percentage

seurat_object[[]]


VlnPlot(seurat_object, features = c("nCount_RNA", "nFeature_RNA", "percentage_mt"))
```

- Visualise a scatter plot of the proportion of mitochondrial UMIs against the library size in each cell.

```{r}
FeatureScatter(seurat_object, "nCount_RNA", "percentage_mt")
```
...
seurat_after_qc <- subset(x = seurat_object, 
    cells = WhichCells(seurat_object, expression = nCount_RNA>4500 & percentage_mt<15 & nFeature_RNA>1500))
...
```{r}
head(FetchData(seurat_after_qc, vars = "LYZ", slot = "counts"))

Assays(seurat_after_qc)

ggplot_lyz_raw <- ggplot(FetchData(object = seurat_after_qc, vars = "LYZ", slot = "counts"), aes(x = LYZ)  ) +
    geom_histogram(   ) +
...
ggplot_lyz_normalised <- ggplot( FetchData(object = seurat_after_qc,
                                           vars = "LYZ", slot = "data"), aes(x = LYZ)  ) +
    geom_histogram(  ) +
    coord_cartesian(ylim = c(0, 500)) +
    cowplot::theme_cowplot()
...
seurat_after_qc <- ScaleData(seurat_after_qc,
                             vars.to.regress = c("nCount_RNA", "percentage_mt")
...
GetAssayData(seurat_after_qc, slot = "scale.data")[1:3, 1:3]
# GetAssayData(seurat_object, slot = "scale.data")[1:3, 1:3] #this won't work
```
...
```{r}
#seurat_after_qc <- RunPCA(seurat_after_qc) ###will also work
...
DimPlot(seurat_after_qc,
        reduction = "pca")
#DimPlot(seurat_after_qc) #This will also work since "pca" is the only thing here, there isn't UMAP or TSNE
```
...
```{r}
ElbowPlot( seurat_after_qc, ndims = 100, reduction = "pca"  )
```
...
```{r}
seurat_after_qc <- RunUMAP(seurat_after_qc, dims = 1:20, reduction = "pca") #based on previous elbow plot
...
#SNN is shared nearest neighbour
...
seurat_after_qc <- FindClusters(
  seurat_after_qc,
  resolution = 0.5,
  random.seed = 0) #the higher the resolution, the smaller the cluster
```
...
#there are methods that can assess how "good" clustering is, e.g. LISI, but not necessarily biology friendly, need to match what you are expecting, i.e. this is hypothesis generating but needs lab validation.
```
...
> Answer: Data frame. (note need to save into new object)
> 
