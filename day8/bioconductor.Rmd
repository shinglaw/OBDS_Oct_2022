...
```{r}
IlluminaQuality(0:40) #lower case letters are higher quality
```
...
```{r}
Ebox <- "CANNTG"
countPattern(Ebox, chrY, fixed = FALSE) #fixed = F means N will be interpreted as any base, T means there must be an N in the genome at that place
```
...
> Answer:
> #if packages are not loaded in the correct order this will fail

- Use the `mapIds()` method to get the Ensembl identifier for those gene symbols.

```{r}
mapIds(org.Hs.eg.db, human_symbols, "ENSEMBL", "SYMBOL", multiVals = "first")
```
...
```{r}
listMarts() #lists what is available today on the BioMart webpage
```
...
```{r}
listEnsemblArchives()
#Archived databases available today on Biomart; note the webpage is important
```
...
```{r}
assay_counts <- read.csv("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/summarized_experiment/counts.csv", row.names=1)
assay_counts
...
```{r}
demo_se <- SummarizedExperiment(
  assays = list(count = assay_counts),
