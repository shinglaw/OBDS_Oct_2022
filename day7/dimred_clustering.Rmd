...
```{r}
pca_iris_species <- 
  ggplot(pca_iris_dataframe, aes(x = PC1, y = PC2, colour = Petal.Length, shape = Species)) +
  geom_point(size = 3)
pca_iris_species
```
...
```{r}
pca_iris$x
pca_iris_unique <- unique(pca_iris$x)
pca_iris_unique

set.seed(1) # Set a seed for reproducible results
tsne_iris <- Rtsne(pca_iris_unique,pca=FALSE,perplexity=30,theta=0.0 )
head(tsne_iris)
str(tsne_iris)
```
...
cowplot::plot_grid(plot_ward, plot_average, plot_complete, plot_single)
...
```{r}
dbscan_plot <- ggplot(iris_clusters_dataframe, aes(iris_clusters_dataframe, x = Sepal.Length, y = Sepal.Width, color = dbscan)    ) +
  geom_point()
...
hdbscan_plot <- ggplot(iris_clusters_dataframe, aes(iris_clusters_dataframe, x = Sepal.Length, y = Sepal.Width, color = hdbscan)    ) +
  geom_point()
...
plot_hdbscan <- ggplot(iris_clusters_dataframe, aes(iris_clusters_dataframe, x = Sepal.Length, y = Sepal.Width, color = hdbscan)    ) +
  geom_point()

plot_dbscan <- ggplot(iris_clusters_dataframe, aes(iris_clusters_dataframe, x = Sepal.Length, y = Sepal.Width, color = dbscan)    ) +
  geom_point()

cowplot::plot_grid(plot_dbscan, plot_hdbscan
...
```{r}
set.seed(1) # Set a seed for reproducible results
kmeans_iris <- kmeans(iris_features, centers, iter.max = 10, nstart = 1   )
kmeans_iris
```
